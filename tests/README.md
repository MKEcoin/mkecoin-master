...
Core tests take longer than any other MKEcoin tests, due to the high amount of computational work involved in validating core components.
...
To run only MKEcoin's core tests (after building):
...
To run only MKEcoin's crypto tests (after building):
...
```bash
MKcoind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `MKEcoin-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only MKEcoin's hash tests (after building):
...
To run only MKEcoin's performance tests (after building):
...
To run only MKEcoin's unit tests (after building):
