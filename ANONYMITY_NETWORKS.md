# Anonymity Networks with MKEcoin

Currently only Tor and I2P have been integrated into MKEcoin. The usage of
...
system clock is fairly accurate - many peers on the MKEcoin network should have
similar timestamps.
...
only using I2P for broadcasting MKEcoin transactions, the total aggregate of
I2P data would also leak information.

#### Mitigation

There is no current mitigation for the user right now. This attack is fairly
sophisticated, and likely requires support from the internet host of a MKEcoin
user.
